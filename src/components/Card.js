        ...
        <Link to={`https://www.pokemon.com/us/pokedex/${pokemon.name}`}>
            ...
            <div className="pokemon">
                ...
                <div>
                    ...
                    {pokemon.types && (
                        <h5>Type: {pokemon.types.map(({ type: { name } }) => name.charAt(0).toUpperCase() + name.slice(1)).join("/")}</h5>
                    )}
                    {pokemon.abilities && (
                        <h5>Abilities: {pokemon.abilities.map(({ ability: { name } }) => name.charAt(0).toUpperCase() + name.slice(1)).join(", ")}</h5>
                    )}
                </div>
            </div>
        </Link>
